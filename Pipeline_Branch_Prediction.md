# RISC-V Simulator - Quick Start Guide
...
- **`print_mem`** or **`pm`** `<address> <rows>` - Display memory contents
- **`dump_mem`** or **`dm`** `<address> <rows>` - Dump memory to JSON file
- **`add_breakpoint`** `<line_number>` - Add a breakpoint
- **`remove_breakpoint`** `<line_number>` - Remove a breakpoint
- **`exit`** - Exit the VM
